...
def plot_results(data, prediction, path, name, remote_stress=0.025):
    ...
    titles = [
        r"Actual Strain $\varepsilon_{xx}$",
    ...
    print(
        f"Element-wise ground truth range: {gs_element_values.min():.6f} to {gs_element_values.max():.6f}"
    )
    print(
        f"Element-wise prediction range: {pred_element_values.min():.6f} to {pred_element_values.max():.6f}"
    )
    ...
    # Save as CSV format
    np.savetxt(
        os.path.join(data_path, "results.csv"),
    ...
    for i, (ax, strain, title, clims, cmap) in enumerate(
        zip(axes, strains, titles, clim, colormaps)
    ):
        ax.set_aspect("equal")
        ax.set_axis_off()
        ...
        mesh_plot = ax.tripcolor(
            triang, strain, shading="flat", cmap=cmap, vmin=clims[0], vmax=clims[1]
        )
...
def plot_regression(
    actual_strain, predicted_strain, path, name, transparent=False, text_color="black"
):
    """
    ...
    props = dict(
        boxstyle="round",
        facecolor=(1, 1, 1, 0.5) if transparent else "white",
        alpha=0.8,
...
def plot_epochs(
    ...
    print(
        f"Training: {len(training_losses)} epochs, min: {min(training_losses):.6f}, max: {max(training_losses):.6f}"
    )
    print(
        f"Validation: {len(validation_losses)} epochs, min: {min(validation_losses):.6f}, max: {max(validation_losses):.6f}"
    )
...
def save_plots(
    args, train_losses, val_losses, test_losses=None, velo_val_losses=None, postprocess_dir=None
):
    model_name = (
        "model_nl"
        + str(args.num_layers)
...
def create_journal_quality_timing_plots(df, results_folder):
    ...
    gs = fig.add_gridspec(
        3, 2, height_ratios=[1, 1, 1], width_ratios=[1, 1], hspace=0.3, wspace=0.25
    )
    ...
    ax1.set_title(
        "(a) Training Time vs Attention Frequency", fontsize=16, fontweight="bold", pad=20
    )
    ...
    ax2.set_xticklabels(
        [f"{int(calls)} calls" for calls in df["expected_calls"]], rotation=45, ha="right"
    )
    ...
    ax3.plot(x_theory, y_theory, "--", linewidth=3, color=colors[2], label="Linear Fit", alpha=0.8)
    ...
    bars2 = ax4.bar(
        ...
        attention_ops,
        width,
        bottom=[message_passing_ops] * len(df),
    ...
    for i, (mp, att) in enumerate(zip([message_passing_ops] * len(df), attention_ops)):
        total = mp + att
        ax4.text(
            i, total + 5, f"{total:.0f}M", ha="center", va="bottom", fontsize=10, fontweight="bold"
        )
    ...
    bars = ax5.bar(
        x_pos, operations_total, width=0.6, alpha=0.6, color=colors[4], label="Total Operations"
    )

    # Plot line for timing efficiency
    line = ax5_twin.plot(
        x_pos, efficiency, "o-", linewidth=3, markersize=8, color=colors[1], label="Time per MOp"
    )
    ...
    ax5_twin.set_ylabel(
        "Time per Million Operations (ms)", fontsize=14, fontweight="bold", color=colors[1]
    )
    ...
    # Overall title
    fig.suptitle(
        "Computational Complexity Analysis: Global Attention Impact on MeshGraphNet Training",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    print("High-quality plots saved to:")
