...
from src.utils.test_utils import ObjectView, benchmark_inference, evaluate_model, visualize_sample
...
def parse_arguments():
    ...
    parser.add_argument(
        "--num_layers", type=int, default=5, help="Number of message passing layers"
    )
    ...
    parser.add_argument(
        "--dropout_rate", type=float, default=0.21887774707222715, help="Dropout rate"
    )
    ...
    parser.add_argument(
        "--visualize_samples", type=int, default=25, help="Number of test samples to visualize"
    )
...
def load_and_split_dataset(dataset_path, shuffle=True, seed=5):
    ...
    if shuffle:
        print(f"Shuffling dataset with seed={seed} (to match legacy training)...")
    ...
    for key, value in checkpoint.items():
        if key in model_state:
            ...
            if model_state[key].shape == value.shape:
                ...
                print(
                    f"⚠ Skipping {key}: shape mismatch (checkpoint: {value.shape}, model: {model_state[key].shape})"
                )
        else:
...
def main():
    ...
    train_dataset, val_dataset, test_dataset, full_dataset = load_and_split_dataset(dataset_path)
    ...
    benchmark_results = benchmark_inference(
        model, test_dataset, device, stats_list, postprocess_dir, num_runs=args.num_benchmark_runs
    )
    ...
    final_test_loss, final_test_rmse = evaluate_model(
        test_dataset, model, device, stats_list, postprocess_dir
    )
