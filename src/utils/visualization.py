...
def plot_results(data, prediction, path, name, remote_stress=0.025):
    ...
    relative_error_strain = np.abs((pred_strain - gs_strain) / remote_stress) * 100
    ...
    for i, (strain, title, clims, cmap, file_name) in enumerate(zip(strains, titles, clim, colormaps, file_names)):
        ...
        # Add colorbar (skip for second plot)
        if i != 1:
    ...
    # Create separate comparison plots (first 3 only)
    plt.rcParams.update({
        'text.usetex': True,
        'font.family': 'serif',
        'font.serif': ['CMU Serif', 'Computer Modern', 'serif'],
...
def save_plots(args, train_losses, val_losses, test_losses=None, velo_val_losses=None, postprocess_dir=None):
    ...
    if postprocess_dir is None:
        postprocess_dir = os.path.join(os.getcwd(), 'results', 'plots')
