...
# Try to import torch (optional - only needed for saving geometry tensor)
import os
...
try:
    ...
    print("WARNING: torch not available in Abaqus Python")
    print("Will skip saving geometry tensor (not critical for pipeline)")
    print("The .inp files will still be generated successfully!")
...
# N = 25  # 25 different hole position configurations (FULL DATASET)
# M = 20  # 20 different displacement values (FULL DATASET)
# Small test: 10 samples
N = 2  # 2 different hole position configurations
M = 5  # 5 different displacement values
...
displacement_values = np.linspace(1, 3, num=M)  # 10 displacement loads
...
def Create_Plates_rec(L, W, hole_radius, hole_x, hole_y, displacement, i, jobname):
        ...
        mymodel.materials['ABQ_PLY_FABRIC'].Depvar(deleteVar=16, 
        n=16)
        mymodel.materials['ABQ_PLY_FABRIC'].UserMaterial(mechanicalConstants=(27500.0, 27500.0, 0.11, 2900.0, 27500.0, 27500.0, 
        ...
        compositeLayup.CompositePly(suppressed=False, plyName='Ply-1', region=region1, 
            material='ABQ_PLY_FABRIC', thicknessType=SPECIFY_THICKNESS, 
            thickness=0.25, orientationType=SPECIFY_ORIENT, orientationValue=45.0, 
        ...
        timePeriod=0.01, nlgeom=OFF, improvedDtMethod=ON)
        ...
        mymodel.TabularAmplitude(name='Ramp', timeSpan=TOTAL, 
        smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (0.01, 1.0)))

        mymodel.DisplacementBC(name='BC-2', createStepName='Step-1', 
        region=region, u1=displacement, u2=UNSET, u3=UNSET, ur1=UNSET, ur2=UNSET, 
        ur3=UNSET, amplitude='Ramp', fixed=OFF, distributionType=UNIFORM, 
        ...
        'PEEQ', 'LE', 'SDV'), numIntervals=1, position=NODES)
