...
MATERIAL_PROPERTIES = {
    ...
    'mechanical_constants': MATERIAL_CONSTANTS
}
...
def create_plate_model(L, W, hole_radius, hole_x, hole_y, displacement, jobname):
    ...
    t = p.MakeSketchTransform(sketchPlane=f1[0], sketchUpEdge=e[3], 
        sketchPlaneSide=SIDE1, sketchOrientation=TOP, origin=(0.0, 0.0, 0.0))  # noqa: F405
    ...
    s1.DistanceDimension(entity1=g[10], entity2=g[3], 
        textPoint=(-49.1842883876953, -43.7521054384766), value=hole_y)
    s1.DistanceDimension(entity1=g[8], entity2=g[3], 
        textPoint=(20.285758609375, -53.3854977724609), value=hole_y)
    s1.DistanceDimension(entity1=g[7], entity2=g[6], 
        textPoint=(80.0, 0.0), value=hole_x)
    s1.DistanceDimension(entity1=g[9], entity2=g[6], 
        textPoint=(-79.0886005214844, -87.3317410585937), value=hole_x)
    ...
    p.PartitionFaceBySketch(sketchUpEdge=e1[3], faces=pickedFaces, 
        sketchOrientation=TOP, sketch=s1)  # noqa: F405
    s1.unsetPrimaryObject()
    ...
    mymodel.materials['ABQ_PLY_FABRIC'].UserMaterial(mechanicalConstants=(
        27500.0, 27500.0, 0.11, 2900.0, 27500.0, 27500.0, 
    ...
    compositeLayup = p.CompositeLayup(
        name='CompositeLayup-1', description='', elementType=SHELL,  # noqa: F405
        offsetType=MIDDLE_SURFACE, symmetric=False,  # noqa: F405
        thicknessAssignment=FROM_SECTION)  # noqa: F405
    compositeLayup.Section(preIntegrate=OFF, integrationRule=SIMPSON,  # noqa: F405
        thicknessType=UNIFORM, poissonDefinition=DEFAULT, temperature=GRADIENT,  # noqa: F405
        useDensity=OFF)  # noqa: F405
    compositeLayup.ReferenceOrientation(orientationType=GLOBAL, localCsys=None,  # noqa: F405
        fieldName='', additionalRotationType=ROTATION_NONE, angle=0.0,  # noqa: F405
        axis=AXIS_3)  # noqa: F405
    compositeLayup.suppress()
    compositeLayup.CompositePly(suppressed=False, plyName='Ply-1', region=region1, 
        material='ABQ_PLY_FABRIC', thicknessType=SPECIFY_THICKNESS,  # noqa: F405
        thickness=0.25, orientationType=SPECIFY_ORIENT, orientationValue=45.0,  # noqa: F405
        additionalRotationType=ROTATION_NONE, additionalRotationField='',  # noqa: F405
        axis=AXIS_3, angle=0.0, numIntPoints=1)  # noqa: F405
    ...
    mymodel.ExplicitDynamicsStep(name='Step-1', previous='Initial', 
        timePeriod=0.01, nlgeom=OFF, improvedDtMethod=ON)  # noqa: F405
    ...
    mymodel.TabularAmplitude(name='Ramp', timeSpan=TOTAL,  # noqa: F405
        smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (0.01, 1.0)))  # noqa: F405

    mymodel.DisplacementBC(name='BC-2', createStepName='Step-1', 
        region=region, u1=displacement, u2=UNSET, u3=UNSET, ur1=UNSET, ur2=UNSET,  # noqa: F405
        ur3=UNSET, amplitude='Ramp', fixed=OFF, distributionType=UNIFORM,  # noqa: F405
        fieldName='', localCsys=None)

    # Define mesh properties
    elemType1 = mesh.ElemType(elemCode=S4R, elemLibrary=STANDARD,  # noqa: F405
        secondOrderAccuracy=OFF, hourglassControl=DEFAULT)  # noqa: F405
    ...
    mymodel.fieldOutputRequests['F-Output-1'].setValues(
        variables=('PEEQ', 'LE', 'SDV'), numIntervals=1, position=NODES)  # noqa: F405
    
    mdb.Job(name=jobname, model='Model-1', description='',  # noqa: F405
        type=ANALYSIS, atTime=None, waitMinutes=0, waitHours=0, queue=None,  # noqa: F405
        memory=90, memoryUnits=PERCENTAGE, explicitPrecision=SINGLE,  # noqa: F405
        nodalOutputPrecision=SINGLE, echoPrint=OFF, modelPrint=OFF,  # noqa: F405
        contactPrint=OFF, historyPrint=OFF, userSubroutine='', scratch='',  # noqa: F405
        resultsFormat=ODB, numDomains=1, activateLoadBalancing=False,  # noqa: F405
        numThreadsPerMpiProcess=1, multiprocessingMode=DEFAULT, numCpus=1)  # noqa: F405
