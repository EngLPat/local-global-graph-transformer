"""
Prepare Training Data for FCLGA GraphTransformer.
...
def load_preprocessed_data():
    ...
    gnn_data = torch.load(config_paths.NODE_DATA, weights_only=False)
    strains = torch.load(config_paths.STRAINS_TENSOR, weights_only=False)
    
    # Try both .pt and .pkl for geometry data
    if config_paths.GEOMETRY_TENSOR.exists():
        plate_geometry_data = torch.load(config_paths.GEOMETRY_TENSOR, weights_only=False)
    elif config_paths.GEOMETRY_PICKLE.exists():
        with open(config_paths.GEOMETRY_PICKLE, 'rb') as f:
    ...
    with open(config_paths.TRIANGULATION_DATA, 'rb') as f:
...
def create_node_features(node_positions, hole_x, hole_y, hole_radius, displacement, fixed_threshold=0.7):
    """
...
def make_graph_bidirectional(edge_index, node_positions):
    ...
    edge_index_set = set((edge_index[0, i].item(), edge_index[1, i].item()) 
...
def process_dataset(gnn_data, strains, plate_geometry_data, triangulation_data):
    ...
    edge_set = set((sample_edge_index[0, j].item(), sample_edge_index[1, j].item()) 
                   for j in range(num_edges))
    
    needs_reverse_edges = any(
        (dst, src) not in edge_set 
        for src, dst in [(sample_edge_index[0, j].item(), sample_edge_index[1, j].item()) 
                        for j in range(num_edges)]
    )
    ...
    for i, (strain, data) in enumerate(zip(strains, gnn_data)):
        ...
        mesh_pos = torch.tensor(
            np.column_stack((triangulation.x, triangulation.y)), 
            dtype=torch.float
        )
        ...
        if i == 0:
            ...
            print(f"  Geometry: L={L:.2f}, W={W:.2f}, radius={hole_radius:.2f}, displacement={displacement:.4f}")
...
def visualize_features(data_list, output_path):
    ...
    # Plot each feature
    for idx, (feature_idx, feature_name) in enumerate([(2, features[1]), 
                                                         (3, features[2]), 
                                                         (4, features[3]), 
                                                         (5, features[4])], start=1):
        sc = axes[idx].scatter(
            node_positions[:, 0], 
            node_positions[:, 1],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Prepare complete training dataset with node and edge features'
    )
    parser.add_argument(
        '--visualize',
        action='store_true',
        help='Generate feature visualization plot'
    )
    args = parser.parse_args()
    
    print("=" * 70)
    print("FCLGA Training Data Preparation")
    print("=" * 70)
    ...
    # Process dataset
    processed_data_list = process_dataset(gnn_data, strains, plate_geometry_data, triangulation_data)
    
    # Save processed data
    output_path = config_paths.DATASETS_DIR / 'processed_data.pt'
    ...
    if args.visualize:
        viz_path = config_paths.RESULTS_DIR / 'preprocessing' / 'feature_visualization.png'
