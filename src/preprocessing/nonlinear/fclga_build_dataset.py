...
STRAINS_DIR = PROJECT_ROOT / "data" / "interim" / "nonlinear" / "strains"
...
def create_strains_tensor(input_folder, node_gnn_data_path):
    ...
    # Regex matches the number at the end: E11_Plate_nonlinear_0.txt -> 0
    files = sorted(
        input_path.glob("E11_*.txt"), 
        key=lambda x: int(re.search(r'_(\d+)\.txt$', x.name).group(1)) 
                      if re.search(r'_(\d+)\.txt$', x.name) else 0
    )
    ...
    for i, file_path in enumerate(files, 1):
        ...
        if missing_count > 0:
            print(f"    INFO: {missing_count}/{len(node_ids)} nodes have no strain data (expected for boundary nodes)")
