"""Run Abaqus FEA simulations in parallel for elastic composite plate geometries.
...
ELASTIC CASE: Uses Static (implicit) solver defined in .inp files.
...
def run_simulation(inp_file, simulations_dir, temp_dir, project_root):
    ...
    command = f'abaqus job={job_name} input="{inp_file}" scratch="{job_temp_dir}" interactive'
    ...
    for extension in keep_extensions:
        for search_path in [job_temp_dir, project_root]:
            ...
            if extension == ".odb":
                print(f"  Warning: {job_name}{extension} not found (simulation may have failed)")
...
def main():
    ...
    print("=" * 80)
    print("ELASTIC CASE: RUNNING ABAQUS SIMULATIONS")
    print("=" * 80)
    ...
    print(
        f"\nRunning {len(inp_files)} simulations with {NUM_PARALLEL_SIMULATIONS} parallel workers\n"
    )
    print("-" * 80)
