...
print("  Material: Elastic fabric composite (orthotropic)")
...
def generate_configurations(N, min_pos=0.3, max_pos=0.7):
    ...
    for x, y in zip(hole_x_factors, hole_y_factors):
        configurations.append({"hole_x_factor": x, "hole_y_factor": y})
...
def create_plate_model(L, W, hole_radius, hole_x, hole_y, displacement, sample_id, jobname):
    ...
    # Add dimensions
    s1.DistanceDimension(entity1=g[10], entity2=g[3], textPoint=(-49.18, -43.75), value=hole_y)
    s1.DistanceDimension(entity1=g[8], entity2=g[3], textPoint=(20.29, -53.39), value=hole_y)
    s1.DistanceDimension(entity1=g[7], entity2=g[6], textPoint=(80.0, 0.0), value=hole_x)
    s1.DistanceDimension(entity1=g[9], entity2=g[6], textPoint=(-79.09, -87.33), value=hole_x)
    ...
    p.PartitionFaceBySketch(sketchUpEdge=e1[3], faces=pickedFaces, sketchOrientation=TOP, sketch=s1)
    s1.unsetPrimaryObject()
    ...
    compositeLayup.CompositePly(
        ...
        orientationValue=PLY_ORIENTATIONS[0],
        additionalRotationType=ROTATION_NONE,
    ...
    compositeLayup.CompositePly(
        ...
        thickness=PLY_THICKNESS,
        orientationType=SPECIFY_ORIENT,
        orientationValue=PLY_ORIENTATIONS[1],
        additionalRotationType=ROTATION_NONE,
        additionalRotationField="",
        axis=AXIS_3,
        angle=0.0,
    ...
    compositeLayup.CompositePly(
        ...
        thickness=PLY_THICKNESS,
        orientationType=SPECIFY_ORIENT,
        orientationValue=PLY_ORIENTATIONS[2],
        additionalRotationType=ROTATION_NONE,
        additionalRotationField="",
        axis=AXIS_3,
        angle=0.0,
    ...
    compositeLayup.CompositePly(
        ...
        thickness=PLY_THICKNESS,
        orientationType=SPECIFY_ORIENT,
        orientationValue=PLY_ORIENTATIONS[3],
        additionalRotationType=ROTATION_NONE,
        additionalRotationField="",
        axis=AXIS_3,
        angle=0.0,
    ...
    elemType1 = mesh.ElemType(
        elemCode=S4R, elemLibrary=STANDARD, secondOrderAccuracy=OFF, hourglassControl=DEFAULT
    )
    ...
    mymodel.FieldOutputRequest(
        name="F-Output-1", createStepName="Step-1", variables=("E",), position=NODES
    )
    ...
    for config_idx, config in enumerate(configurations):
        ...
        for disp_idx, displacement in enumerate(displacement_values):
            ...
            if sample_id % 50 == 0 or sample_id == 0:
                ...
                print(
                    f"  Config {config_idx + 1}/{N_HOLE_CONFIGS}, Disp {disp_idx + 1}/{N_DISPLACEMENTS}"
                )
