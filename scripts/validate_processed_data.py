...
def load_data():
    ...
    if node_data_path.exists():
        ...
        if len(data["node_gnn_data"]) > 0:
            ...
            if hasattr(sample, "node_ids"):
                print("  - Node IDs available: Yes")
            else:
                print("  - Node IDs available: No (may cause ordering issues!)")
    else:
    ...
    if tri_data_path.exists():
        ...
        print("✓ Loaded triangulation_data.pkl")
    ...
    if strains_path.exists():
        ...
        print("✓ Loaded strains.pt")
        print(f"  - Tensor shape: {data['strains'].shape}")
        print("  - [num_samples, max_nodes, features]")
    ...
    if geom_path.exists():
        ...
        print("✓ Loaded plate_geometry_data.pt")
        print(f"  - Shape: {data['geometry'].shape}")
        print("  - [num_samples, 6] = [L, W, hole_radius, hole_x, hole_y, displacement]")
    elif geom_pkl_path.exists():
        with open(geom_pkl_path, "rb") as f:
            data["geometry"] = pickle.load(f)
        print("✓ Loaded plate_geometry_data.pkl")
        print(f"  - Number of samples: {len(data['geometry'])}")
    else:
        print("⚠ No geometry data found (optional)")
...
def plot_mesh_geometry(data, sample_idx, save_path):
    ...
    for i in range(0, edge_index.shape[1], 10):  # Plot every 10th edge to avoid clutter
        ...
        if node_start < len(x_coords) and node_end < len(x_coords):
            ax1.plot(
                [x_coords[node_start], x_coords[node_end]],
    ...
    ax2.set_title(f"Sample {sample_idx}: Node Density Distribution", fontsize=14, fontweight="bold")
...
def plot_strain_field(data, sample_idx, save_path):
    ...
    scatter = ax2.scatter(
        x_coords, y_coords, c=valid_strains, cmap="RdYlBu_r", s=20, edgecolors="none"
    )
    cbar2 = plt.colorbar(scatter, ax=ax2, label="E11 Strain")
    ax2.set_xlabel("X Coordinate", fontsize=12)
    ax2.set_ylabel("Y Coordinate", fontsize=12)
    ax2.set_title(
        f"Sample {sample_idx}: E11 Strain Distribution (Nodes)", fontsize=14, fontweight="bold"
    )
    ax2.set_aspect("equal")
...
def plot_statistics(data, save_path):
    ...
    if "geometry" in data:
        ...
        axes[1, 1].text(
            0.5, 0.5, "No geometry data available", ha="center", va="center", fontsize=14
        )
        axes[1, 1].axis("off")
...
def plot_edge_connectivity(data, sample_idx, save_path):
    ...
    valid_plus_one = np.sum(
        (edge_plus_one[0] < num_actual_nodes) & (edge_plus_one[1] < num_actual_nodes)
    )
    valid_minus_one = np.sum(
        (edge_minus_one[0] >= 0)
        & (edge_minus_one[1] >= 0)
        & (edge_minus_one[0] < num_actual_nodes)
        & (edge_minus_one[1] < num_actual_nodes)
    )

    # Create 3x2 plot
    fig, axes = plt.subplots(3, 2, figsize=(20, 24))
    fig.suptitle(
        f"Edge Connectivity Analysis - Sample {sample_idx}", fontsize=16, fontweight="bold"
    )
...
def compare_with_legacy_dataset(data):
    ...
    print(
        "    Expected: [num_nodes, 6] = [x, y, is_hole_edge, is_fixed, is_displaced, displacement]"
    )

    print(f"  • Edge index: {sample.edge_index.shape}")
    print("    Expected: [2, num_edges]")
    print("    Format: edge_index[0] = source nodes, edge_index[1] = target nodes")

    if hasattr(sample, "edge_attr"):
        print(f"  • Edge attributes: {sample.edge_attr.shape}")
        print("    Expected: [num_edges, 3] = [dx, dy, distance]")
    else:
        print("  • Edge attributes: NOT FOUND")

    print(f"  • Target (y): {sample.y.shape}")
    print("    Expected: [num_nodes, 1] = strain values")
    ...
    if edge_index.max() >= sample.x.shape[0]:
        print("  ✗ CRITICAL: Edge indices exceed number of nodes!")
        print(f"    Max edge index: {edge_index.max()}, Num nodes: {sample.x.shape[0]}")
        print("    This will cause wrong node features to be used!")
    else:
        print("  ✓ Edge indices within bounds")
    ...
    if non_zero.std() < 1e-6:
        print("  ⚠ WARNING: Very low strain variance - check if data is correct!")
...
def validate_data_consistency(data):
    ...
    if num_samples_gnn != num_samples_strains:
        issues.append(
            f"⚠ Mismatch: GNN data has {num_samples_gnn} samples but strains has {num_samples_strains}"
        )
    ...
    if "node_gnn_data" in data and len(data["node_gnn_data"]) > 0:
        ...
        if "strains" in data:
            ...
            print("Node vs non-zero strain counts (first 10):")
            ...
            large_mismatches = sum(
                1 for n, s in strain_sample_sizes if abs(n - s) > n * 0.05
            )  # >5% missing
            if large_mismatches > 0:
                issues.append(
                    f"⚠ {large_mismatches}/10 samples have significant node-strain count mismatch (>5%)"
                )
            else:
                print("  ℹ Minor strain data gaps detected (expected for boundary nodes)")
...
def main():
    ...
    parser.add_argument(
        "--sample", type=int, default=0, help="Starting sample index to visualize (default: 0)"
    )
    parser.add_argument(
        "--num-samples", type=int, default=3, help="Number of samples to visualize (default: 3)"
    )
    parser.add_argument(
        "--no-plots", action="store_true", help="Skip generating plots (only check data)"
    )
