...
def run_preprocessing(args, project_root):
    ...
    src_preprocessing = os.path.join(project_root, "src", "preprocessing")
    geometry_dir = os.path.join(project_root, "data", "raw", "geometry")
    simulations_dir = os.path.join(project_root, "data", "raw", "simulations")
    strains_dir = os.path.join(project_root, "data", "interim", "strains")
    ...
    print("\nYou can now run: python scripts/fclga_train.py\n")
...
Examples:
  ...
  python fclga_run_pipeline.py --stage all
  
  # Run only preprocessing
  python fclga_run_pipeline.py --stage preprocess --num_cpus 8
  
  # Run only training
  python fclga_run_pipeline.py --stage train
  
  # Run only testing
  python fclga_run_pipeline.py --stage test
        """,
    )
    parser.add_argument(
        "--stage",
    ...
    if args.stage in ["all", "preprocess"]:
        success = run_preprocessing(args, project_root)
