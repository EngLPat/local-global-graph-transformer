...
def plot_results(data, prediction, path, name, remote_stress=0.025):
    ...
    for i, (strain, title, clims, cmap, file_name) in enumerate(zip(strains, titles, clim, colormaps, file_names)):
        ...
        plot_path = os.path.join(plots_path, f"{name}_{file_name}.svg")
        plt.savefig(plot_path, format='svg', bbox_inches='tight', transparent=True)
    ...
    for i, (strain, title, clims, cmap, filename) in enumerate(zip(combined_strains, combined_titles, combined_clim, combined_colormaps, combined_filenames)):
        ...
        comparison_path = os.path.join(path, f"{name}_comparison_{filename}.svg")
        plt.savefig(comparison_path, format='svg', bbox_inches='tight', transparent=True)
        plt.close(fig)
    ...
    plot_regression(gs_strain, pred_strain, path, name, transparent=True, text_color='black')
...
def plot_regression(actual_strain, predicted_strain, path, name, transparent=True, text_color='black'):
    ...
    if transparent:
        ax_reg.patch.set_alpha(0)  # Transparent axes background
    else:
        ax_reg.set_facecolor('white')
    ...
    ax_reg.plot(
        ...
        [actual_min, actual_max],
        color=text_color, linestyle='--', linewidth=2, label="Ideal prediction"
    )
    ...
    reg_path = os.path.join(path, name + '_regression.svg')
    plt.savefig(reg_path, format='svg', bbox_inches='tight', transparent=transparent)
    plt.close(fig_reg)
...
def plot_epochs(train_loss_path, val_loss_path, output_dir, name="training_validation_loss", transparent=True, text_color='black'):
    """
    ...
    plot_path = os.path.join(output_dir, f"{name}.svg")
    plt.savefig(plot_path, format='svg', bbox_inches='tight', transparent=transparent)
    plt.close(fig)
...
def save_plots(args, train_losses, val_losses, test_losses=None, velo_val_losses=None):
    ...
    PATH = os.path.join(args.postprocess_dir, model_name + '.svg')
...
best_params = {
    ...
    'batch_size': 1,
    'hidden_dim': 48,
    'dropout_rate': 0.22205772151439881,
    'epochs': 3000,  # Set as needed
    'opt': 'rmsprop',
    'opt_scheduler': 'step',  # Or as used in your code
    'opt_decay_step': 48,
    'opt_decay_rate': 0.7677125372992852,
    'opt_restart': 0,
    'weight_decay': 4.174983096810349e-05,
    'lr': 0.0005030580320977311,
...
def plot_physical_consistency(results, path, name):
    ...
    # Save plot
    plot_path = os.path.join(path, f"{name}_physical_consistency.svg")
    plt.savefig(plot_path, format='svg', bbox_inches='tight', transparent=True)
    plt.close(fig)
...
model_path = 'results/0_standard_20251102_213632/best_models/model_nl5_bs4_hd48_ep3000_wd4.174983096810349e-05_lr0.0005030580320977311_shuff_True.pt'
